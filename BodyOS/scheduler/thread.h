...
typedef struct {
    uint8_t priority;       // thread priority (unique)
    volatile state_t state; // thread state
    tcb_t *entry;            // the first task to be executed
    tcb_t *next;             // the current task to be executed
    tcb_t *task_queue[10];
    int q_head;
    int q_tail;
    buffer_t buffer;        // holds task shared persistent variables
...
#pragma PERSISTENT(__persistent_vars)
#define __shared(...)   \
        typedef struct {    \
            __VA_ARGS__     \
        } FRAM_data_t  __attribute__ ((aligned (2)));    \
        static FRAM_data_t __persistent_vars[2];
